  ...
  (let (block-name
    ...
    (dolist (v vars)
	  ...
	  (progn
	    ...
	    (setq l (cddr lets)))))
    (dolist (i iters)
      (let ((outdb (if (member (car i) '(:on :eq))
	    ...
	    (listcheck `(progn
			  ...
			  nil))
	    (middb `(destructuring-bind (,(cadr i))
			,(if (eq (car i) :from)
			     `(list ,(cadddr i))
			     `(list (car ,(cadddr i))))
		      nil))
	    (indb `(destructuring-bind (,(cadr i))
		       ,(if (eq (car i) :from)
			    `(list ,(caddr i))
			    `(list (car ,(caddr i))))
		     nil)))
		  ...
		  (block ,middle-block-name
		    ,(let ((pre-initially-forms-body
			    `(progn
				 ...
				 (tagbody
				    ,tagname
				    (if (or ,@(mapcar (lambda (x) (when (not (eq (car x) :eq))
								    (if (eq (car x) :from)
									(if (elt x 4)
									    (if (= (elt x 5) 1)
										`(> ,(elt x 2) ,(elt x 4))
					    ...
					    `(progn
					       ...
					       ,@(mapcar (lambda (x) (cond
									...
									`(setq ,(elt x 2) (+ ,(elt x 1) ,(* (elt x 5) (elt x 6)))))
								       (t
									`(setq ,(elt x 2) (funcall ,(elt x 4) ,(elt x 2)))))) iters)
					       (go ,tagname))))
				       (if indestbinds
					   (progn
