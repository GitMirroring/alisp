...
(defmacro in-package (pack)
  `(eval-when (:compile-toplevel :load-toplevel :execute)
     (setq *package* (find-package ',pack))))
...
(defun require (mod &optional pl)
  (unless (member mod *modules* :test #'string=)
    (cond
      ...
      ((string= mod "COMPILER")
       (load (concatenate 'string al:*module-path* "compiler.lisp")))
      (t
...
(defun integer-length (num)
  (values (ceiling (log (if (minusp num)
			    (- num)
			    (1+ num))
			2))))
...
(defmacro define-modify-macro (name lambda-list func &optional doc)
  (let (args restvar)
    ...
    `(defmacro ,name (place . ,lambda-list)
	 ...
	 (setq letf (cons (list (caaddr exp) (,(if restvar 'list* 'list) ',func (nth 4 exp) ,@args ,@(if restvar (list restvar)))) letf))
  ...
  (let (name opt constrname constrs constrdefs copier pred slotnames funcdefs constr-prov-p
	...
	(setq funcdefs (append
			`((defun ,accname (struct)
			    (slot-value struct ',sl))
			  (defun (setf ,accname) (newval struct)
...
(defmacro multiple-value-setq (vars valform)
  (let ((valssym (gensym)))
    `(let ((,valssym (multiple-value-list ,valform)))
       ,@(let (setqs (i 0))
	   (dolist (v vars)
	     (setq setqs (cons `(setq ,v (nth ,i ,valssym)) setqs))
	     (incf i))
	   (reverse setqs))
       (car ,valssym))))
...
(defmacro defpackage (s &rest opts)
  (let ((name (string s))
	nicks uses import-first shadows imports exports intern)
    (dolist (opt opts)
      (case (car opt)
	(:nicknames (setq nicks (append nicks (mapcar #'string (cdr opt)))))
	(:documentation)
	(:use (setq uses (append uses (mapcar #'find-package (cdr opt)))))
	(:shadow (setq shadows (append shadows (mapcar #'string (cdr opt)))))
	(:shadowing-import-from
	 (setq import-first (append import-first (list (list (find-package (cadr opt)) (mapcar #'string (cddr opt))))))
	 (setq shadows (append shadows (mapcar #'string (cddr opt)))))
    ...
    `(eval-when (:compile-toplevel :load-toplevel :execute)
	 ...
	 (make-package ,name))
       ...
       ,@(mapcar (lambda (l) `(import (list ,@(mapcar (lambda (sm) `(intern ,sm ,(car l))) (cadr l))) ,name)) import-first)
       (shadow ',shadows ,name)
...
(defun loop-parse-accumulation (forms ifclvar accvar)
  (let ((sym (string (car forms)))
    ...
    (cond
      ...
      ((or (string= sym "MAXIMIZE")
	   (string= sym "MAXIMIZING"))
       (setq out `(if ,var
		      (setq ,var (max ,var ,val))
		      (setq ,var ,val))))
      ((or (string= sym "MINIMIZE")
	   (string= sym "MINIMIZING"))
       (setq out `(if ,var
		      (setq ,var (min ,var ,val))
		      (setq ,var ,val)))))
    (if (string= (string (caddr forms)) "INTO")
...
(defun loop-parse-iteration (forms)
  (let (var iter)
    ...
    (if (member (car forms) '(fixnum float t nil))
	(setq forms (cdr forms))
	(if (and (symbolp (car forms))
		 (string= (car forms) "OF-TYPE"))
	    (setq forms (cddr forms))))
    (do ((f forms))
	((not f) (setq forms f))
      (if (atom (car f))
	  (let ((sym (string (car f))))
	    (cond
		 ...
		 (setq iter `(:from ,var 0 nil > ,1)))
	       (cond
		 ((eq (car iter) :from)
		  (setf (elt iter 5) (* (elt iter 5) (cadr f))))
		 ((member (car iter) '(:in :on))
		  (setf (elt iter 4) (cadr f))))
	       (setq f (cddr f)))
	      ((string= sym "IN")
	       (setq iter `(:in ,var nil ,(cadr f) ,'#'cdr))
	       (setq f (cddr f)))
	      ((string= sym "ON")
	      ...
	      ((string= sym "BEING")
		   ...
		   (setq f (cdr f)))
	       (cond
		 ((or (string= (string (car f)) "HASH-VALUE")
		      (string= (string (car f)) "HASH-VALUES"))
		  (setq iter `(:hashtable nil ,var nil ,nil)))
		 ((or (string= (string (car f)) "HASH-KEY")
		      (string= (string (car f)) "HASH-KEYS"))
		  (setq iter `(:hashtable ,var nil nil ,nil)))
		 ((or (string= (string (car f)) "PRESENT-SYMBOL")
		      (string= (string (car f)) "PRESENT-SYMBOLS"))
		  (setq iter `(:package ,var nil nil nil ,nil)))
		 ((or (string= (string (car f)) "SYMBOL")
		      (string= (string (car f)) "SYMBOLS"))
		  (setq iter `(:package nil ,var nil nil ,nil)))
		 ((or (string= (string (car f)) "EXTERNAL-SYMBOL")
		      (string= (string (car f)) "EXTERNAL-SYMBOLS"))
		  (setq iter `(:package nil nil ,var nil ,nil))))
	       (setq f (cdr f))
	  ...
	  (progn
	    (setq forms f)
	    (return nil))))
    (if (member (car iter) '(:in :on :across))
	...
	(setf (elt iter 2) (gensym)))
      (setf (elt iter 3) (gensym)))
    (when (eq (car iter) :package)
      (setf (elt iter 5) (gensym)))
    (values iter forms)))
  ...
  (let (block-name
	...
	(if (symbolp form)
		...
		((string= sym "NEVER")
		 ...
		 (setq forms (cddr forms)))
		((string= sym "WHILE")
		 (setq do-forms (append do-forms `((if (not ,(cadr forms)) (return-from ,inner-block-name ,nil)))))
		 (setq forms (cddr forms)))
		((string= sym "UNTIL")
		 (setq do-forms (append do-forms `((if ,(cadr forms) (return-from ,inner-block-name ,nil)))))
		 (setq forms (cddr forms)))
		((or (string= sym "COLLECT")
		     (string= sym "COLLECTING")
		     (string= sym "APPEND")
	...
	(when (eq (car i) :from)
	  (let ((stopvar (gensym))
		(stepvar (gensym)))
	    (setq vrs (list* (elt i 1) (list stopvar (elt i 3)) (list stepvar (elt i 5)) vrs))
	    (setq initial-setup `(,@(if (elt i 3)
					(list `(when (,(elt i 4) ,(elt i 1) ,stopvar)
						 (setq ,skip-body-sym t)
						 (go ,prologue-tag))))
				    (setq ,(elt i 1) ,(elt i 2))
				    . ,initial-setup))
	    (setq iteration-setup `(,@(if (elt i 3)
					  (list `(if (,(elt i 4) ,(elt i 1) ,stopvar)
	...
	(when (eq (car i) :package)
	  (setq vrs (list* (or (elt i 1) (elt i 2) (elt i 3)) (elt i 5) vrs))
	  (setq initial-setup `((al:loopy-setq ,(or (elt i 1) (elt i 2) (elt i 3)) (car ,(elt i 5)))
				(when (endp ,(elt i 5))
				  (setq ,skip-body-sym t)
				  (go ,prologue-tag))
				(setq ,(elt i 5)
				      (let ((pack (find-package ,(or (elt i 4)
								     '*package*)))
					    out)
					(do-symbols (sym pack)
					  ,(cond
					     ((elt i 1)
					      `(if (not (eq
							 (nth-value 1 (find-symbol (string sym) pack))
							 :inherited))
						   (setq out (cons sym out))))
					     ((elt i 2)
					      `(setq out (cons sym out)))
...
(defun format-returning-unconsumed-args (out fstr &rest args)
  (let ((*standard-output* (if (not out)
	...
	(progn
	  (apply fstr out args)
	  nil)
	(let (in-spec at-sign colon sign num dirargs iterbegin otherargs skip-mode
		      capit-only-first-word
		      (capit-depth 0)
	  ...
	  (labels ((print-and-clear-string-buffer-if-possible nil
		   ...
		   (convert-case (s)
		     (if (or (stringp s)
			     (symbolp s)
			     (characterp s))
			 (funcall case-conv-primitive (string s))
			 s)))
	    (do ((i 0 (+ 1 i)))
		((= i (length fstr)))
	      (let ((ch (elt fstr i)))
		(if in-spec
		    (cond
			 ...
			 (fresh-line))
		      ...
		      ((char= ch #\()
		       (if (= 0 capit-depth)
			   (setq capit-only-first-word nil
				 case-conv-primitive (cond
						       ((and at-sign colon)
							#'string-upcase)
						       (at-sign
							(setq capit-only-first-word t)
							#'string-capitalize)
						       (colon
							#'string-capitalize)
						       (t
...
(defgeneric describe-object (obj str))
(defun inspect (obj))
...
(dolist
    (sym '(*features* *default-pathname-defaults* defmacro in-package defun
           ...
           require lambda-parameters-limit call-arguments-limit
           multiple-values-limit array-rank-limit array-dimension-limit
           array-total-size-limit char-code-limit lambda-list-keywords first
           second third fourth fifth sixth seventh eighth ninth tenth rest caar
